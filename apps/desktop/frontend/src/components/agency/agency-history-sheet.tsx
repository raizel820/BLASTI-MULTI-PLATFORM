...
export type { HistoryReservation, ServiceItem, HistoryResponse };
...
const STATUS_MAP: Record<string, { label: TranslationKeys; className: string }> = {
  ...
  POSTPONED: {
    label: 'statusPostponed' as TranslationKeys,
...
export { formatTime, getLocalizedName };

// ─── AgencyHistoryContent ────────────────────────────────────────────────────
// Reusable content component used by both AgencyHistorySheet (drawer) and
// the full-page History page.

export function AgencyHistoryContent({ active }: { active: boolean }) {
  const { t } = useLanguage();
  ...
  }, [active, fetchHistory, fetchServices]);

  // Re-fetch when page/filters change
  useEffect(() => {
    if (active) fetchHistory();
  }, [page, debouncedSearch, statusFilter, serviceFilter, active, fetchHistory]);
...
// ─── AgencyHistorySheet ──────────────────────────────────────────────────────
// Sheet/drawer wrapper that delegates to AgencyHistoryContent.
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="w-full max-w-2xl p-0 flex flex-col gap-0 overflow-hidden">
        ...
        <AgencyHistoryContent active={open} />
      </SheetContent>
    </Sheet>
  );
}
