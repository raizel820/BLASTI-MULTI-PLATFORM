...
// ─── Upload Base URL ─────────────────────────────────────────────────────────
// On Desktop, uploads go to the local API at localhost:3080

const isDevServer = typeof window !== 'undefined' &&
  (window.location.port === '5173' || window.location.port === '3000');
const UPLOAD_BASE = isDevServer ? '' : 'http://127.0.0.1:3080';
  ...
  const upload = useCallback(
    async (file: File, metadata?: Record<string, string>): Promise<UploadState> => {
      ...
      try {
        ...
        // Use XMLHttpRequest with progress tracking, targeting local API
        const result = await new Promise<{
          ...
          xhr.open('POST', `${UPLOAD_BASE}/api/upload?type=${type}`);
