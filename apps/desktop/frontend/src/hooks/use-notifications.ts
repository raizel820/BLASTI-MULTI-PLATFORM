...
export function useNotifications(userId?: string): UseNotificationsReturn {
  ...
  const fetchNotifications = useCallback(async (append = false) => {
    ...
    try {
      ...
      const apiNotifs = (res?.notifications ?? []).map(mapApiNotification);
  ...
  // On Desktop, skip polling when sync store reports offline
  useEffect(() => {
    let stopped = false;
    ...
    const tick = async () => {
      ...
      // Skip entirely when fully offline (Desktop: check sync store)
      if (!useSync.getState().isOnline) {
  ...
  const markAllAsRead = useCallback(async () => {
    ...
    try {
      await api.request('/api/notifications/read-all', { method: 'PUT', body: { markAll: true } });
    } catch {
