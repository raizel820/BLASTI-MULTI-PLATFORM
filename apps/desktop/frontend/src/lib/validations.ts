...
export const subscriptionPaySchema = z.object({
  plan: z.enum(['PREMIUM', 'PRO'], { message: 'Plan must be PREMIUM or PRO' }),
  method: z.enum(['CCP', 'BANK', 'ELECTRONIC'], { message: 'Payment method must be CCP, BANK, or ELECTRONIC' }),
 ...
 * Returns parsed data or an error object.
 *
 * Note: Desktop version returns a plain error object instead of NextResponse
 * since there is no next/server in the Electron environment.
 */
export function validateBody<T extends z.ZodType>(
  schema: T,
  body: unknown
): { data: z.infer<T>; error: null } | { data: null; error: { status: number; body: { success: false; error: string; details: Array<{ field: string; message: string }> } } } {
