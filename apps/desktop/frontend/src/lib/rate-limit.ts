 ...
 *
 * Desktop adaptation: Removed Next.js dependencies (NextRequest, NextResponse).
 * Uses generic Request/Response instead.
 */
...
/**
 * Gets the client IP from a standard Request object.
...
export function getClientIp(request: Request): string {
  ...
  // Use user-agent as a secondary identifier
 ...
 *
 * @returns the client IP string for further use
 * @throws RateLimitError or IpBlockedError
 */
export function enforceRateLimit(request: Request, options: RateLimitOptions): string {
 ...
 * Returns a standard Response object (not NextResponse).
 */
export function rateLimitErrorResponse(error: unknown): Response {
  if (error instanceof IpBlockedError) {
    return new Response(
      JSON.stringify({ success: false, error: error.message, retryAfter: error.retryAfter }),
...
/** Kiosk join: 20 joins per 5 minutes per IP */
...
/** Kiosk read: 60 requests per minute per IP */
